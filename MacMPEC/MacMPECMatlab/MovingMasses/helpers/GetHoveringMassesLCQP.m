...
% Algebraic variables
y = SX.sym('y', 1, 1);               % Switching values
lambda0 = SX.sym('lambda0', 1, 1);   % Refering to negative parts of v
lambda1 = SX.sym('lambda1', 1, 1);   % Refering to positive parts of v
l = SX.sym('l', 1, 1);               % Refering to 1 - y  
...
% vdot = ... 
f_x = [f_x; -2*p(1) + p(2) - v(1)];
for i=2:nMasses-1
    f_x = [f_x; p(i-1) - 2*p(i) + p(i+1) - v(i)];
end
% Last mass has control
f_x = [f_x; p(nMasses-1) - 2*p(nMasses) - v(nMasses) - 0.3*(2*y - 1) + u];
...
% algebraic equations
f_z = [v(nMasses) - (lambda0  - lambda1); l - (1 - y)];
