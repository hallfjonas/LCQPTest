...
param r := if n == 8 then 1.6E4		# penalty parameter
           else if n == 16 then 3E5	# ... depends on n
           else if n == 32 then 5E5
           else if n == 64 then 1E6
           else 0;
...
minimize area: h/2 * sum{i in 1..n} ( a[i] + a[i-1] )
               + r*h*h * sum{i in Omega0} (u[i] - xi[i]);
