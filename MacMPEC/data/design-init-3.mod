...
param det := x[3]^2*x[6]^2+x[4]^2*x[5]^2-2*x[3]*x[5]*x[4]*x[6];
param r11 :=  ( x[5]^2+x[6]^2 ) / det;
param r12 := -( x[3]*x[5]+x[4]*x[6] ) / det;
param r22 :=  ( x[3]^2+x[4]^2 ) / det;
...
subject to
   ...
   ball{k in K}: r11*(y[1,k] - x[1])^2 + 2*r12*(y[1,k] - x[1])*(y[2,k] - x[2]) 
	                               +   r22*(y[2,k] - x[2])^2 <= 1;
...
problem init1:
	g1,				# ... objective
	{i in J} y[i,1],	 	# ... variables
	ball[1];			# ... constraints
problem init2:
	g2,				# ... objective
	{i in J} y[i,2],	 	# ... variables
	ball[2];			# ... constraints
problem init3:
	g3,				# ... objective
	{i in J} y[i,3],	 	# ... variables
	ball[3];			# ... constraints
